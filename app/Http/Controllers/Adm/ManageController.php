...
class ManageController extends Controller {
    ...
    public function postGetConst(Request $request)
    {
        ...
        if (empty($request['w'])) {
            ...
            $json = Cnt::whereW($request['w'])->get();
            return response()->json(['result' => TRUE, 'data' => $json]);
        }

    }

    public function postSetConst(Request $request)
    {

        $VAR = $request->all();
        if (!empty($VAR['w'])) {

            return $this->_setConst($request, Cnt::whereW($VAR['w'])->get());
        } else {
    ...
    public function postSaveConst(Request $request)
    {
        $VAR = $request->all();

        if (empty($VAR['price'])) {
            Cnt::find($request->input('id'))
            ...
            FlowerPacket::find($request->input('id'))
                ->update(array('title' => $request->input('title'), 'price' => $request->input('price')));
            return response()->json(['result' => TRUE]);
        }
    ...
    public function _setConst(Request $request, $w)
    {
        ...
        Cnt::create(['title' => $request->input('title'), 'w' => $request->input('w')]);
        return response()->json(['result' => TRUE]);
    }
